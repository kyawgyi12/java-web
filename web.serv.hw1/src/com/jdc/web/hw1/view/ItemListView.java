...
public class ItemListView extends AbstractView {
	...
	protected void writeView(HttpServletRequest req, HttpServletResponse resp)
		...
		if(list.size() > 0) {
			...
			Tag panelHead = TagFactory.create(new BasicTagFactory("div", "Item List"));
			...
			form.addAttribute("action", "item");
			colDiv.addChilds(form);
			
			Tag table = TagFactory.create(new BasicTagFactory("table"));
			table.addAttribute("class", "table table-striped");
			
			Tag button = TagFactory.create(new BasicTagFactory("input"));
			button.addAttribute("class", "btn btn-default");
			button.addAttribute("type", "submit");
			button.addAttribute("value", "Add To Cart");
			form.addChilds(table, button);
			
			Tag tr = TagFactory.create(new BasicTagFactory("tr"));
			Tag td1 = TagFactory.create(new BasicTagFactory("td"));
			Tag check = TagFactory.create(new BasicTagFactory("input"));
			...
			for(Item item : list) {
				...
				Tag check_1 = TagFactory.create(new BasicTagFactory("input"));
				check_1.addAttribute("type", "checkbox");
				check_1.addAttribute("name", "item[]");
				check_1.addAttribute("value", String.valueOf(item.getId()));
				td1_1.addChilds(check_1);
				Tag td2_1 = TagFactory.create(new BasicTagFactory("td", item.getName()));
				Tag td3_1 = TagFactory.create(new BasicTagFactory("td", String.valueOf(item.getPrice())));
